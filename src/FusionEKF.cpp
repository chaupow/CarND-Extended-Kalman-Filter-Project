...
FusionEKF::FusionEKF() {
  ...
  */
  // [Done] by
  // Lesson 25: Extended Kalman Filters
...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      ekf_.x_ << std::max<double>(rho * cos(phi), threshold),
                 std::max<double>(rho * sin(phi), threshold),
                 std::max<double>(rho_dot * cos(phi), threshold),
                 std::max<double>(rho_dot * sin(phi), threshold);
    }
